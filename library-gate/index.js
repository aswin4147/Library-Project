...
// --- API Routes (Login, Logout, etc. - No changes here) ---

// POST /api/login ... (code is correct)
...
// POST /api/logout ... (code is correct)
...
// GET /api/check-status ... (code is correct)
app.get("/api/check-status", requireLogin, (req, res) => {
  res.json({ success: true, username: req.session.username });
});

// POST /api/student-details ... (code is correct)
...
// POST /api/punch ... (code is correct)
app.post("/api/punch", requireLogin, async (req, res) => {
  ...
  try {
    if (action === "punch_in") {
      ...
      res.json({
        success: true,
        message: `Student ${register_number} punched IN for ${purpose}.`,
      });
      ...
      if (result.affectedRows > 0) {
        res.json({
          success: true,
          message: `Student ${register_number} punched OUT.`,
        });
      } else {
        res.json({
          success: false,
...
app.get("/api/history", requireLogin, async (req, res) => {
  ...
  // **MODIFIED**: Added s.department to the SELECT statement
  let sql = `
        SELECT 
  ...
  if (year) {
    whereClauses.push("YEAR(v.punch_in_time) = ?");
    params.push(year);
  }
  if (month) {
    whereClauses.push("MONTH(v.punch_in_time) = ?");
    params.push(month);
  }
  if (day) {
    whereClauses.push("DAY(v.punch_in_time) = ?");
    params.push(day);
  }
  if (purpose) {
...
app.get("/api/history/export", requireLogin, async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "No records found for the given filters to export." });
    }

    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Library Visit History");

    // **MODIFIED**: Added the Department column to the Excel export
    worksheet.columns = [
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=library_history_report.xlsx"
    );
...
// --- NEW: GET /api/students - Fetch all students ---
